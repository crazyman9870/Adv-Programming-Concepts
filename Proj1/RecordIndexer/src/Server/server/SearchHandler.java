...
public class SearchHandler implements HttpHandler {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		try {
			...
			SearchIn param = (SearchIn)xmlStream.fromXML(exchange.getRequestBody());
			...
			if(check.getUser() != null) {
				SearchOut result = serverFacade.search(param);
				exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0);
				xmlStream.toXML(result, exchange.getResponseBody());
				exchange.getResponseBody().close();
			}
			else {
				SearchOut result = new SearchOut(null, 1);
			...
			System.out.println("Failed in the SearchHandler");
			e.printStackTrace();
