...
public class GetFieldsHandler implements HttpHandler {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		try {
			...
			GetFieldsIn param = (GetFieldsIn)xmlStream.fromXML(exchange.getRequestBody());
			...
			if(check.getUser() != null) {
				GetFieldsOut result = serverFacade.getFields(param);
				exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0);
				xmlStream.toXML(result, exchange.getResponseBody());
				exchange.getResponseBody().close();
			}
			else {
				GetFieldsOut result = new GetFieldsOut(null, 1);
