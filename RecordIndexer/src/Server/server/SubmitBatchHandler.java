...
public class SubmitBatchHandler implements HttpHandler {
	...
	public void handle(HttpExchange exchange) throws IOException {
		...
		try {
			...
			SubmitBatchIn param = (SubmitBatchIn)xmlStream.fromXML(exchange.getRequestBody());
			...
			if(check.getUser() != null && check.getUser().getBatchKey() != -1) {
				SubmitBatchOut result = serverFacade.submitBatch(param);
				exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0);
				xmlStream.toXML(result, exchange.getResponseBody());
				exchange.getResponseBody().close();
			}
			else {
				SubmitBatchOut result = new SubmitBatchOut(1);
			...
			System.out.println("Failed in the SubmitBatchHandler");
			e.printStackTrace();
